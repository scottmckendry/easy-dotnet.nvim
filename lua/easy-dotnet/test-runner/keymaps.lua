...
---@param unit_test_results easy-dotnet.test.RunResult[]
---@param win table
---@param node easy-dotnet.test.runner.Node
...
local function get_test_result_handler(win, node, on_job_finished)
  ---@param rpc_res easy-dotnet.test.RunResult[]
...
function M.test_run(node, win, cb)
  ...
  local tests = {}
  ---@param child easy-dotnet.test.runner.Node
  ...
  local filter = vim.tbl_map(function(test)
    ---@type easy-dotnet.test.RunRequestNode
    return {
      uid = test.id,
